...
def get_market_data(limit=50): params = {"start": "1", "limit": limit, "convert": "USD"} response = requests.get(cmc_url, headers=headers, params=params) if response.status_code == 200: data = response.json()["data"] return pd.DataFrame([{ "Name": coin["name"], "Symbol": coin["symbol"], "Price": coin["quote"]["USD"]["price"], "Change(24h)": coin["quote"]["USD"]["percent_change_24h"] } for coin in data]) else: st.error(f"API Error: {response.status_code}") st.code(response.text, language="json") return pd.DataFrame()
...
Simulated Buyer/Seller Ratio (random logic)
...
refresh = st.button("ڈیٹا دوبارہ لوڈ کریں (Auto Refresh)") if refresh or "df" not in st.session_state: st.session_state.df = get_market_data(50)

Layout

col1, col2 = st.columns([1, 2]) with col1: df = st.session_state.df selected = st.selectbox("کوائن منتخب کریں:", df["Name"] if not df.empty else []) if selected: coin_info = df[df["Name"] == selected].iloc[0] symbol = coin_info["Symbol"] price = coin_info["Price"] change = coin_info["Change(24h)"]

with col2: if selected: st.subheader(f"{selected} کا لائیو چارٹ:") iframe(f"https://www.tradingview.com/widgetembed/?symbol=BINANCE:{symbol}USDT&interval=15&hidesidetoolbar=1", height=420)
